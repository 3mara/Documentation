...
In this project we tried to implement semantic similarity techniques on arabic text, where we used the corpus based techniques and the knowledge based one.Then we clustered the news based on their relative similarities so we could be able to recommend to the user articles based on her interest and where it fit in our clusters.
Finally experiments showed that our approache perform better than pure lexical similarities, However there is some cases where the result of our approaches differ by small value from the lexical one, we beleive that results can be enhanced by more paramter tuning trials for clustering techniques.
